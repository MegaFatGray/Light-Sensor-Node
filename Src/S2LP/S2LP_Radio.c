...
#define FBASE_DIVIDER           1048576.0         /*!< 2^20 factor dividing fxo in fbase formula */

#define VCO_CENTER_FREQ          3600.0           /*!< VCO center frequency in MHz */
...
#define CHSPACE_DIVIDER         32768.0           /*!< 2^15 factor dividing fxo in channel space formula */

#define MINIMUM_DATARATE                 100  /*!< Minimum datarate supported by S2LP1 100 bps */
#define MAXIMUM_DATARATE                 500000  /*!< Maximum datarate supported by S2LP1 500 kbps */

#define FDEV_DIVIDER                    (float)524288.0
...
#define IS_DATARATE(DATARATE, F_CLK)      (DATARATE>=MINIMUM_DATARATE && DATARATE<=(MAXIMUM_DATARATE*(F_CLK/1e6)/26))


#define F_DEV_LOWER_LIMIT(F_Xo)		((F_Xo/FDEV_DIVIDER)/8)
#define F_DEV_UPPER_LIMIT(F_Xo)		(787109*((F_Xo/1e6)/26))
#define IS_F_DEV(FDEV,F_Xo)        (FDEV>=F_DEV_LOWER_LIMIT(F_Xo) && FDEV<=F_DEV_UPPER_LIMIT(F_Xo))

#define CH_BW_LOWER_LIMIT(F_CLK)      (1100*(F_CLK/1e6)/26)  /*!< Minimum value of the channel filter bandwidth */
#define CH_BW_UPPER_LIMIT(F_CLK)    (800100*(F_CLK/1e6)/26)  /*!< Maximum value of the channel filter bandwidth */
...
void S2LPRadioSearchDatarateME(uint32_t lDatarate, uint16_t* pcM, uint8_t* pcE)
{
  ...
  fXtalFreq = (float)s_lXtalFrequency;
  
  if(s_lXtalFrequency>DIG_DOMAIN_XTAL_THRESH) {
    fXtalFreq /= 2.0;
  }
  
  s_assert_param(IS_DATARATE(lDatarate, fXtalFreq));
  
  fXtalDiv = (double)fXtalFreq/4294967296.0;

  /* Search the exponent value */
  for(uDrE = 0; uDrE != 12; uDrE++) {
    lDatarateTmp = S2LPRadioComputeDatarate(65535, uDrE);
  ...
  *pcE = (uint8_t)uDrE;
  ...
  if(uDrE==0) {
    *pcM = (uint32_t)round(lDatarate/fXtalDiv);
  }
  else {
    *pcM = (uint32_t)round( (((lDatarate/fXtalDiv)/pow(2, uDrE-1))- 65536) );
  }
...
void S2LPRadioSearchFreqDevME(uint32_t lFDev, uint8_t* pcM, uint8_t* pcE)
{
  ...
  fXtalDiv = ((float)s_lXtalFrequency)/(FDEV_DIVIDER*bs);
  ...
  *pcE = (uint8_t)uFDevE;
  ...
  if(uFDevE==0) {
    *pcM = (uint8_t)round(((float)lFDev/fXtalDiv)/((bs*refdiv)/8.0));
  } else {
    *pcM = (uint8_t)round((((float)lFDev/fXtalDiv)/((bs*refdiv*pow(2,uFDevE-1))/8.0)) -256);
  }
...
uint32_t S2LPRadioComputeDatarate(uint16_t cM, uint8_t cE)
{
  ...
  fXtalDiv = (double)s_lXtalFrequency;
  
  if(s_lXtalFrequency>DIG_DOMAIN_XTAL_THRESH) {
    fXtalDiv /= 2.0;
  }  
  fXtalDiv /= 4294967296.0;
  
  if(cE==0) {
    return (uint32_t)round(fXtalDiv*cM);
  } else {
    return (uint32_t)round( fXtalDiv*(cM+65536)*pow(2, cE-1) );
  }
}
...
{
  float fXtalDiv;
  fXtalDiv = ((float)s_lXtalFrequency)/(FDEV_DIVIDER*bs*refdiv);
  
  if(cE==0) {
    return (uint32_t)round(fXtalDiv*round((bs*cM*refdiv)/8.0));
  }
  else {
    return (uint32_t)round(fXtalDiv*round((bs*(cM+256)*refdiv*pow(2,cE-1))/8.0));
  }
...
uint32_t S2LPRadioComputeChannelFilterBw(uint8_t cM, uint8_t cE)
{
  float tmpXtalFreq = (float)s_lXtalFrequency;
  
  if(s_lXtalFrequency>DIG_DOMAIN_XTAL_THRESH) {
    tmpXtalFreq /= 2.0;
  }
  return (uint32_t)round(100.0*s_vectnBandwidth26M[cM+(cE*9)]*(tmpXtalFreq/26e6));
...
uint32_t S2LPRadioComputeFrequencyBase(uint32_t lSynthWord, uint8_t bs, uint8_t refdiv)
{
  return (uint32_t)round(lSynthWord*(((double)s_lXtalFrequency)/(FBASE_DIVIDER*refdiv*(bs/2.0))));
}
...
uint32_t S2LPRadioComputeSynthWord(uint32_t frequency, uint8_t refdiv)
{
  ...
  return (uint32_t)round(((double)frequency*(band/2.0))*(((double)FBASE_DIVIDER*refdiv)/s_lXtalFrequency));
}
...
uint8_t S2LPRadioComputeChannelSpacingRegValue(uint32_t lChannelSpace)
{
  return (uint8_t)round((((float)lChannelSpace)/s_lXtalFrequency)*CHSPACE_DIVIDER);
}
...
uint32_t S2LPRadioComputeChannelSpacing(uint8_t cChSpaceRegVal)
{
  return (uint32_t)round((((float)s_lXtalFrequency)/CHSPACE_DIVIDER)*cChSpaceRegVal);
}
...
{
  uint32_t nFreqDig = s_lXtalFrequency;
  
  if(s_lXtalFrequency>DIG_DOMAIN_XTAL_THRESH) {
    nFreqDig = s_lXtalFrequency/2;
  }
  
  *pcAnaIf = (uint8_t)(((nIF*2048)/(((float)s_lXtalFrequency)/12.0))-100);
  *pcDigIf = (uint8_t)(((nIF*2048)/(((float)nFreqDig)/12.0))-100);
}
...
*       power value.
*/
void S2LPRadioSetPALeveldBm(uint8_t cIndex, float fPowerdBm)
{
  ...
  if(fPowerdBm> 13.5) { 
    ...
    paLevelValue = (uint8_t)((MAX_PA_VALUE-fPowerdBm)/PA_STEP)+1;
  }
...
*       power value.
*/
float S2LPRadioGetPALeveldBm(uint8_t cIndex)
{
  uint8_t address, paLevelValue;
  s_assert_param(IS_PA_MAX_INDEX(cIndex));
  ...
  return (float)(MAX_PA_VALUE-((paLevelValue-1)*PA_STEP));
}
