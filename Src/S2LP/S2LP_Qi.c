...
#define IS_RSSI_DBM(VAL)              ((VAL>=-146.0) && (VAL<=-2.5))
#define IS_RSSI_FLT(VAL)              (VAL<=15)
#define IS_PQI_LVL_CHECK(VAL)         (VAL<=15)
#define S2LPRadioRegToRssidBm(VAL)    ((float)VAL - 146.0)
#define S2LPRadioRssidBmToReg(VAL)    (uint8_t)(VAL+146.0)
 ...
 */
float S2LPRadioGetRssidBm(void)
{
  uint8_t tmp;
  g_xStatus = S2LPSpiReadRegisters(RSSI_LEVEL_ADDR, 1, &tmp);  
  return S2LPRadioRegToRssidBm(tmp);
}
 ...
 * @retval uint8_t RSSI value.
 */
float S2LPRadioGetRssidBmRun(void)
{
  uint8_t tmpBuffer[2];
  ...
  return S2LPRadioRegToRssidBm(tmpBuffer[1]);
}
 ...
 * @retval uint8_t RSSI value.
 */
void S2LPRadioSetRssiThreshdBm(float rssiThrehsold)
{
  uint8_t tmp;
  s_assert_param(IS_RSSI_DBM(rssiThrehsold));
  
  tmp = S2LPRadioRssidBmToReg(rssiThrehsold);
